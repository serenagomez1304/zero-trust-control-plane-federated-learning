"""
Car Rental Agent — ZTA Multi-Agent Testbed
============================================
...
AGENT_ID = os.getenv("AGENT_ID", "car-rental-agent")
AGENT_NAME = os.getenv("AGENT_NAME", "Car Rental Agent")
...
AGENT_CARD = AgentCard(
    name=AGENT_NAME, description="Searches rental vehicles, books rentals, retrieves and cancels reservations.",
    ...
    skills=[
        AgentSkill(id="vehicle-search", name="Vehicle Search",
            description="Search for available rental vehicles at a location for given dates",
            tags=["cars", "vehicles", "rental", "search", "travel"],
            examples=["Find cars in LAX from July 1 to July 5"]),
        AgentSkill(id="rental-booking", name="Rental Booking",
            description="Book a vehicle, retrieve rental details, or cancel a reservation",
            tags=["booking", "reservation", "cancel"],
            examples=["Book the Toyota Camry for John Doe"]),
        AgentSkill(id="rental-info", name="Rental Information",
            description="List locations, vehicle categories, and available add-ons",
            tags=["locations", "categories", "add-ons"],
            examples=["What pickup locations are available?"]),
    ],
)
...
SYSTEM_PROMPT = """You are the Car Rental Agent, a specialist in vehicle rentals.
You have tools: search_vehicles, book_vehicle, get_rental, get_rental_by_confirmation, cancel_rental, list_locations, list_vehicle_categories, list_add_ons.
...
async def lifespan(app: FastAPI):
    ...
    mcp_config = {"car-rental-mcp": {"url": f"{MCP_SERVER_URL}/sse", "transport": "sse"}}
    try:
...
async def identity():
    return {"agent_id": AGENT_ID, "agent_name": AGENT_NAME, "agent_type": "worker",
            "domain": "car-rental", "protocol": "a2a"}
